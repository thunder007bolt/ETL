"""
Pipeline faits — domaine GRH.
Charge les tables de faits via fact_config.
"""
...
from domaines.grh.facts.fact_config import FACT_CONFIG
...
logger = logging.getLogger("cnss_etl.grh.pipeline_facts")
...
class FactsPipeline:
    ...
    def run(self, batch_date: datetime = None) -> None:
        ...
        try:
            ...
            logger.exception("Erreur pipeline GRH FACTS")
            raise
        finally:
            src_conn.close()
            dw_conn.close()
