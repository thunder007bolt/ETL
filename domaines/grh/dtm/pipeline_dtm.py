"""
Pipeline DTM — alimente les tables d'agrégats DTM du domaine GRH (multi-cliché).
...
logger = logging.getLogger("cnss_etl.grh.pipeline_dtm")
...
class DtmPipeline:
    ...
    def run(self, names: list[str] = None, batch_date: datetime = None) -> None:
        """
        Charge les tables DTM GRH spécifiées (toutes si names=None).
        ...
        cliche     = f"{batch_date.month:02d}{batch_date.year}"   # MMYYYY
        ...
        try:
            ...
            logger.info(f"Pipeline DTM GRH terminé — {rows_loaded} lignes chargées")

        except Exception:
            logger.exception("Erreur pipeline DTM GRH")
            raise
        finally:
            dw_conn.close()
...
class _DtmLoader(BaseLoader):
    """Loader DTM GRH — wrapping BaseLoader pour les tables d'agrégats DTM."""
