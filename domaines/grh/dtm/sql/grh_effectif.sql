...
ANNEES AS (
    ...
    WHERE s.CLICHE        = :1
      ...
      AND EXTRACT(YEAR FROM s.DATE_DEBUT) BETWEEN 1900 AND EXTRACT(YEAR FROM SYSDATE)
),
REF AS (
    SELECT
        ANNEE,
...
BASE AS (
    ...
    INNER JOIN DWH.FAIT_GRH_PERSONNE p
        ...
        AND p.CLICHE     = :1
        AND p.DATE_NAISS IS NOT NULL
        AND EXTRACT(YEAR FROM p.DATE_NAISS) BETWEEN 1900 AND EXTRACT(YEAR FROM SYSDATE)
