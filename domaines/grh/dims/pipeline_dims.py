"""
Pipeline dimensions — domaine GRH.
Charge les tables de dimensions via dim_config.
"""
...
from domaines.grh.dims.dim_config import DIM_CONFIG
...
logger = logging.getLogger("cnss_etl.grh.pipeline_dims")
...
class DimsPipeline:
    ...
    def run(self, batch_date: datetime = None) -> None:
        ...
        try:
            ...
            logger.info(f"Pipeline GRH DIMS terminé — {rows_loaded} lignes chargées")

        except Exception:
            logger.exception("Erreur pipeline GRH DIMS")
            raise
        finally:
            src_conn.close()
            dw_conn.close()
