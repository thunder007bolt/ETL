...
-- Grain   : ID_TEMPS × TIPP × DR_NO × SP_NO × LP_NO × SEXE
-- R1      : TIPP depuis FAIT_TSINISTRE.TIPP (0/1 direct)
...
base AS (
    SELECT
        TO_NUMBER(TO_CHAR(TRUNC(sin.DOS_DATE_ACCIDENT, 'MM'), 'YYYYMMDD'))  AS ID_TEMPS,
        EXTRACT(YEAR FROM sin.DOS_DATE_ACCIDENT)                            AS ANNEE,
